...
public:
    ...
    virtual void setSampler(class ISampler * sampler) override final { m_sampler = sampler; }

    /// Implementation of ITextureView::GetSampler()
    virtual ISampler * getSampler() override final { return m_sampler.get(); }
    const ISampler *   getSampler() const { return m_sampler.get(); }

    /// Implementation of ITextureView::GetTexture()
    virtual ITexture * getTexture() override final { return m_texture; }
    const ITexture *   getTexture() const { return m_texture; }

    template<typename TextureType>
    TextureType * getTexture()
    {
        return static_cast<TextureType>(m_texture);
    }
